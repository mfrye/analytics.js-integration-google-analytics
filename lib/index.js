...
var TRACKER_NAME = 'newTracker';

// Whether to create and use a different tracker
var createNewTracker = false;
...
GA.prototype.initialize = function() {
  ...
  // Set global
  createNewTracker = opts.createNewTracker;
  ...
  if (createNewTracker) {
    gaConfig.name = TRACKER_NAME;
  }

  window.ga('create', opts.trackingId, gaConfig);

  // Big picture intelligence
  if (opts.companyIntel) {
    window.ga(cmd('require'), 'BigPicture');
  }

  // display advertising
  if (opts.doubleClick) {
    window.ga(cmd('require'), 'displayfeatures');
  }

  // https://support.google.com/analytics/answer/2558867?hl=en
  if (opts.enhancedLinkAttribution) {
    window.ga(cmd('require'), 'linkid', 'linkid.js');
  }

  // send global id
  if (opts.sendUserId && user.id()) {
    window.ga(cmd('set'), 'userId', user.id());
  }

  // anonymize after initializing, otherwise a warning is shown
  // in google analytics debugger
  if (opts.anonymizeIp) window.ga(cmd('set'), 'anonymizeIp', true);

  // custom dimensions & metrics
  var custom = metrics(user.traits(), opts);
  if (len(custom)) window.ga(cmd('set'), custom);
...
GA.prototype.page = function(page) {
  ...
  // set
  window.ga(cmd('set'), { page: pagePath, title: pageTitle });

  if (this.pageCalled) delete pageview.location;

  // send
  window.ga(cmd('send'), 'pageview', pageview);
...
GA.prototype.identify = function(identify) {
  ...
  if (opts.sendUserId && identify.userId()) {
    window.ga(cmd('set'), 'userId', identify.userId());
  }
...
GA.prototype.track = function(track, options) {
  ...
  var dataConfig = contextOpts.dataConfig;
  payload = buildPayload(payload, props, dataConfig);
  ...
  window.ga(cmd('send'), 'event', payload);
};
...
GA.prototype.completedOrder = function(track) {
  ...
  // require ecommerce
  if (!this.ecommerce) {
    window.ga(cmd('require'), 'ecommerce');
    this.ecommerce = true;
  }

  // add transaction
  window.ga(cmd('ecommerce:addTransaction'), {
  ...
  each(products, function(product) {
    ...
    window.ga(cmd('ecommerce:addItem'), {
...
GA.prototype.viewedCheckoutStepEnhanced = function(track) {
  ...
  window.ga(cmd('ec:setAction'), 'checkout', {
    step: props.step || 1,
...
GA.prototype.completedCheckoutStepEnhanced = function(track) {
  ...
  window.ga(cmd('ec:setAction'), 'checkout_option', {
    step: props.step || 1,
    option: options
  });

  window.ga(cmd('send'), 'event', 'Checkout', 'Option');
};
...
GA.prototype.completedOrderEnhanced = function(track) {
  ...
  window.ga(cmd('ec:setAction'), 'purchase', {
    id: orderId,
    affiliation: props.affiliation,
    revenue: total,
    tax: track.tax(),
...
GA.prototype.refundedOrderEnhanced = function(track) {
  ...
  each(products, function(product) {
    var track = new Track({ properties: product });
    window.ga(cmd('ec:addProduct'), {
      id: track.id() || track.sku(),
      quantity: track.quantity()
    });
  });

  window.ga(cmd('ec:setAction'), 'refund', {
    id: orderId
  });
...
GA.prototype.clickedPromotionEnhanced = function(track) {
  ...
  window.ga(cmd('ec:setAction'), 'promo_click', {});
  this.pushEnhancedEcommerce(track);
};
...
function enhancedEcommerceTrackProduct(track) {
  ...
  window.ga(cmd('ec:addProduct'), product);
}
...
function enhancedEcommerceProductAction(track, action, data) {
  ...
  window.ga(cmd('ec:setAction'), action, data || {});
}
...
function cmd(command) {
  return createNewTracker ? TRACKER_NAME + '.' + command : command;
}
...
function buildPayload(payload, properties, dataConfig) {
  if (!dataConfig) return payload;

  for (var key in dataConfig) {
    // Check if the property exists
    if (properties[dataConfig[key]]) {
      payload[key] = properties[dataConfig[key]];
    }
  }
